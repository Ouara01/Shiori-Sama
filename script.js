...
- @returns {string} Requête GraphQL formatée
  */
  ...
  return ` query ($page: Int, $perPage: Int, $sort: [MediaSort]) {   Page(page: $page, perPage: $perPage) { media(type: ${type}, sort: $sort) { id title { userPreferred } coverImage { large medium } startDate { year month day } averageScore popularity season episodes type staff { edges { node { language } } } }   } }`;
  }
...
card.innerHTML = `<img src="${item.image_url}" alt="${item.title}" loading="lazy"> <div class="card-title">${item.title}</div> <div class="card-type">${item.type || "ANIME"}</div> <div class="card-language">${getFlagEmoji(item.language || "JP")}</div> ${item.seasonEpisode ?`<div class="card-info"><span class="season-episode">${item.seasonEpisode}</span></div>`: ""}`;
...
- - FALL (Automne) : Octobre à Décembre
  ...
  const query = `query ($year: Int, $season: MediaSeason, $perPage: Int) { Page(page: 1, perPage: $perPage) { media(seasonYear: $year, season: $season, sort: POPULARITY_DESC, type: ANIME) { title { userPreferred } coverImage { large } siteUrl } } }`;
...
- - Blocage du scroll du body quand le menu est ouvert
  ...
  const mobileMenu = document.querySelector(’.mobile-menu–top’); // ✅ Sélecteur corrigé
  ...
  document.body.classList.toggle(‘no-scroll’); // ✅ Bloque le scroll
  });
  ...
  document.body.classList.remove(‘no-scroll’); // ✅ Restaure le scroll
  });
